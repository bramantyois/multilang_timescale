...
class DataSequence(object):
    ...
    def chunksums(self, interp="rect", **kwargs):
        ...
        if [interp] is "stack", the signal will be stacked for each word in the TR (giving a [num_TRs x (max_words_per_TR* num_dims)] matrix).
        ...
        if interp == "sinc":
            ...
            return self.chunkmeans()
        elif interp == "stack":
            feature_dims = self.data.shape[1]
            outmat = np.zeros(
                (len(self.split_inds) + 1, feature_dims * kwargs["max_words_per_TR"])
            )
            for ci, c in enumerate(self.chunks()):
                if len(c):
                    stacked_features = np.hstack(c)
                    outmat[ci, : len(stacked_features)] = stacked_features
            return outmat
